...
uint8_t Receive_Char_Uart8(St_UART_Typedef *Inst, uint8_t *msg, enum Polling_mechism PollingEn)
{
    // Check RXNE bit : if set meaning the data is transferred to shift register 
    if(PollingEn == Enable)
        while(!(Inst->UART_SR & UART_SR_RXNE));
    ...
    if((Inst->UART_CR1 & UART_CR1_M) == UART_CR1_M)
    {
        if((Inst->UART_CR1 & UART_CR1_PCE) == UART_CR1_PCE)
        {
            *((uint8_t*)msg) = (Inst->UART_DR & (uint8_t)0xFF);
        }else
        {
            *((uint8_t*)msg) = Inst->UART_DR;
        }
    }else
    {
        if((Inst->UART_CR1 & UART_CR1_PCE) == UART_CR1_PCE)
        {
            *((uint8_t*)msg) = (Inst->UART_DR & (uint8_t)0x7F);
        }else
        {
            *((uint8_t*)msg) = (Inst->UART_DR & (uint8_t)0xFF);
        }
 ...
 * @param   Timeout : The time out of receive data by uart
 * @return  uint8_t 1 : Successed
 *                  0 : Failed  
 */
uint8_t Receive_String_Uart(St_Uart_API *Inst, uint8_t *msg, uint8_t msg_len, uint32_t Timeout)
{
    uint8_t *pdata_8bit = NULL;
    uint16_t *pdata_9bit = NULL;
    
    // Check if the buffer points to an address
    if(msg == NULL)
    {
        return 0;
    }

    // Check if receive data length 8bit or 9bit
    if((Inst->Word_Len == Payload_9_bit) && (Inst->Parity == Parity_None))
    {
        pdata_8bit = NULL;
        pdata_9bit = (uint16_t *)msg;
    }else
    {
        pdata_8bit = msg;
        pdata_9bit = NULL;
    ...
    while(msg_len > 0)
    {
        ret = Receive_With_Timeout(Inst, current_tick, Timeout);
        if(ret == 0) { return 0;}
        if(pdata_8bit == NULL)
        {
            *pdata_9bit = (uint16_t)(Inst->UARTx->UART_DR & 0x01FF);
            pdata_9bit++;
        }else
        {
            if(Inst->Parity == Parity_None)
            {
                *pdata_8bit = (uint8_t)(Inst->UARTx->UART_DR & (uint8_t)0xFF);
            }else
            {
                *pdata_8bit = (uint8_t)(Inst->UARTx->UART_DR & (uint8_t)0x7F);
            }
            pdata_8bit++;
        }
        msg_len = msg_len - (uint8_t)1;
    }
...
/**
 * @brief Receive Data by polling method
 * @return  uint8_t 1 : Successed
 *                  0 : Failed Timeout
 */
 
uint8_t Receive_With_Timeout(St_Uart_API *Inst,uint32_t tick_start, uint32_t timeout)
{
    while(!(_Uart_Get_Flag(Inst, UART_SR_RXNE)))
    {
        if(timeout != Max_Delay)
        {
            if((timeout == 0U) || (Get_Tick() - tick_start > timeout))
            {
                return 0;
            }
        }
 ...
 *                  0 : Failed  
 */
uint8_t Send_Text_Uart(St_Uart_API *Inst, void *msg, enum Polling_mechism PollingEn)
{
    uint16_t *msg1 = (uint16_t*)msg;
    uint16_t message;
    while (*msg1 != '\0')
    {
        if(Inst->Word_Len == Payload_8_bit)
        {
            message = *msg1 & (uint16_t)0x00FF;
            Send_Char_Uart(Inst->UARTx,&message,PollingEn);
            message = *msg1 >> 8;
            Send_Char_Uart(Inst->UARTx,&message,PollingEn);
        }else
        {
            Send_Char_Uart(Inst->UARTx,msg1,PollingEn);
        }
        msg1++;
    }
}
