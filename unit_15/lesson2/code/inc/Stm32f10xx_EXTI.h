...
#define NVIC_Set_Pirority           __NVIC_SetIRQn_Pirority
...
static inline void __NVIC_SetIRQn_Pirority(IRQn_Type IRQn, uint32_t pirority)
{
  if((int32_t)(IRQn) >= 0)
  {
    NVIC->IPR0[((uint32_t)IRQn)] = (uint8_t)((pirority << (8U - 3U)) & (uint32_t)0xFFUL);
  }else
  {
    SCB->SHP[(((uint32_t)IRQn) & 0xFUL)-4UL] = (uint8_t)((pirority << (8U - 3U)) & (uint32_t)0xFFUL);
  }
}
