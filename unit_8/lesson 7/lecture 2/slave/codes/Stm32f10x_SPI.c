...
{
    uint16_t ch = 0x000F;
    SPI_Tx_Rx_Char(SPIx,&ch,&ch,SPI_Pol_Disable);
    Send_Char_Uart(UART1, &ch, Enable);
    UNUSED(SPIx);
}
...
{
    if((SPI1->SPI_SR & 1 << RXNE) == 1 << RXNE)
        spi1_config.P_Rx_Empty_CallBack(&spi1_config);   //  jump to defualt Tx_Empty_CallBack function
    else if((SPI1->SPI_SR & 1 << TXE) == 1 << TXE)
        spi1_config.P_Tx_Empty_CallBack(&spi1_config);  //  jump to defualt Rx_Empty_CallBack function
